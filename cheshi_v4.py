    ...
    def contextMenuEvent(self, event):
        if not self.indexOfTopLevelItem(self.currentItem()) == -1:
            ...
            if self.currentItem().childCount()==0:
                twoAction = self.contextmenu.addAction('Compartment Setup')
                twoAction.triggered.connect(self.currentItem().configure)
...
class SurreyWindow(QMainWindow):
    ...
    def newProject(self):
        ...
        self.newprojectPath=QFileDialog.getExistingDirectory(self,'Create your project') #返回一个绝对路径
        ...
        if self.newprojectPath:
            ...
            #
            self.newproject_name = os.path.split(self.newprojectPath)[1]
            self.newproject = ProjectItem(self.projectview, prj_path=self.newprojectPath)  # 父窗口QTreeWidget
    ...
    def openProject(self):
        self.openprojectPath = QFileDialog.getOpenFileName(self, 'Open File','.','(Project File (*.cfg))')[0] #第三个是默认显示路径
        self.openprojectPath = self.openprojectPath.replace('/','\\')
        #print(self.openprojectPath)
        if self.openprojectPath:
            self.openproject_name =os.path.split(os.path.split(os.path.split(self.openprojectPath)[0])[0])[1]
            #print(self.openproject_name)
            self.openproject=ProjectItem(self.projectview, cfg_path=self.openprojectPath)
...
class ProjectItem(QTreeWidgetItem):
    def __init__(self, parent=None, cfg_path=None, prj_path=None):
        ...
        parent.itemDoubleClicked.connect(self.viweData)  # parent在原来中是self.projectview
