...
_conf = config.Config(sys.argv[1])
...
t_start, t_end, Nsteps = [0, 36000, 10]
...
for i in range(Nsteps):
    ...
    newpath = os.path.join(sys.argv[2],str(t_range[i]))
    if not os.path.exists(newpath):
        os.makedirs(newpath)

    # Save current concentrations
    for j in range(nComps):
        fn = os.path.join(newpath, 'comp' + str(j) + '_' + _conf.comps_geom[j].name)
