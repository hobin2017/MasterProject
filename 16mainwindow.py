...
class MyThread(Thread):
    """
    Temporary solution for the simulation. There might be other solutions.
    """
    ...
    def run(self):
        """
        This function will be executed automatically when the start function of this instance is called.
        """
        compDPK('%s'%self.path)
#-------------------------------------------------------------------------------------------------------------
class SurreyWindow(QMainWindow):
    def __init__(self,parent=None):
        ...
        self.mainresultlayout.setCurrentWidget(self.resultviewer1)
        # Mainwindow Layout
        ...
        self.menu_result= self.menu2.addAction('View Result')# the child of menubar2
        self.menu_result.triggered.connect(self.viewResult)
    ...
    def viewResult(self):
        self.csvPath, self.Pro_filter3 = QFileDialog.getOpenFileName(self, 'Select File for result', '.','csv File (*.csv)')
        if self.csvPath:
            if os.name == 'nt':
                self.csvPath = self.csvPath.replace('/','\\')
            self.csvData = pd.read_csv(filepath_or_buffer=self.csvPath,header=1)
            #print(self.csvData.shape[1])
            # Drawing part
            self.resultviewer2.axes.cla()  # Clear the canvas
            colCount = self.csvData.shape[1] #column count of the csv file
            for i in range(2,colCount):
                self.resultviewer2.axes.plot(self.csvData.iloc[:,0],self.csvData.iloc[:,i])
            self.resultviewer2.axes.set_title('Transdermal permeation simulation')
            self.resultviewer2.axes.set_xlabel('Time(s)')
            self.resultviewer2.axes.set_ylabel('Mass fraction')
            forLegend = ['Evaporation', 'VH', 'SC', 'VE', 'DE', 'Receptor fluid']
            self.resultviewer2.axes.legend(forLegend[1:])
            self.resultviewer2.canvas.draw() #Update the canvas
