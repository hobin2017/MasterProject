...
class SurreyWindow(QMainWindow):
    ...
    def NewProject(self):
        ...
        self.newproject=NewProject0(self.projectview) #传入父窗口参数
        ...
        if not os.path.exists('.\\config'):
            os.mkdir('.\\config')
        with open('.\\config\\Nicotine.cfg','w') as self.newproject._file:
    ...
    def Run(self):
        #假设PyQt主程序与run.py在同一目录下
        # 需要别人电脑安装好python，且设置好路径
        # 假设Project文件夹与run.py一起
        runpath=os.path.join(os.path.abspath('.'),'run_v1.py')
        cmd='python '+'"'+runpath+'"'
        output=os.popen(cmd)
        print(output.read()) #读出一堆
...
#--------------------------------------------------------------------------------------------------------
class NewProject0(QTreeWidgetItem):
    def __init__(self, parent=None):
        super(NewProject0, self).__init__(parent)
        # 具体工程结构的搭建
        self.project0 = QTreeWidgetItem(parent)
        ...
        # parent.projectview.itemDoubleClicked.connect(self.Viewer)  # 触发函数
